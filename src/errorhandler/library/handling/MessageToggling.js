	...
	return BaseHandling.extend("errorhandler.library.handling.MessageToggling", {
		...
		toggleControlMessages: function() {
			this.getAllControls()
				...
				.forEach(binding => binding.attachChange(event => {
					this.getAllControls()
						.filter(control => control.getBinding("visible") === event.getSource())
						.forEach(control => this._toggleMessageOfControl(control, control.getVisible()));
				}));
		},
