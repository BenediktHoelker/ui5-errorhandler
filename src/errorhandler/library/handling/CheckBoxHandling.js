sap.ui.define([
	...
	return BaseHandling.extend("errorhandler.library.handling.CheckBoxHandling", {
		...
		showValueStateForCheckBoxes: function() {
			this.getAllControls()
				.filter(control => this.checkIfControlIsType(control, "sap.m.CheckBox"))
				.forEach(checkbox => checkbox.attachSelect(event => {
		...
		_getValueStateOfCheckBox: function(oCheckBox) {
			const aMessages = oCheckBox.getBinding("selected").getDataState().getControlMessages();
